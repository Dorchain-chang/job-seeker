...
function ragPanelHtml(){
  return '<div class="ragsec">'
   ...
   +'<button type="button" class="schip ragchip">哪些岗位 7 天内截止？</button>'
...
function taskRender(){
  ...
  a.forEach(function(k){
    ...
    h+='<div class="taskrow'+(k.done?' done':'')+(over?' over':'')+'">'
      +'<span class="badge">'+(k.done?'已完成':(over?'已逾期':(k.due?(k.due===t?'今天到期':k.due):'无期限')))+'</span>'
      +'<b>'+esc(k.title)+'</b>'
...
function schRender(){
  ...
  if(!rows.length){box.innerHTML='<div class="empty">暂无日程：给投递记录填「下次节点」，或到「定时任务」加一条</div>';return}
  var t=today(),h='';
...
<section id="secAgent" style="display:none">
  ...
  <p style="margin:0 0 10px;font-size:12px;color:var(--sub)">多轮对话：提问时先在本机资料库做 BM25 检索，命中的片段作为上下文交给模型（回答带【序号】出处）；没命中就按求职常识回答 · 未配置 Key 时为演示输出 · 会话存本机（最近 60 条）</p>
